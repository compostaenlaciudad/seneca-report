...
import { POLITICIANS, findPolitician } from '@/lib/data';
...
function ComparePageInner() {
  ...
  const a = findPolitician(params.get('a') ?? 'ricardo-monreal') ?? POLITICIANS[0];
  const b = findPolitician(params.get('b') ?? 'xochitl-galvez')  ?? POLITICIANS[1];

  const scoreDelta = a.score - b.score;
  const colorA = scoreDelta < 0 ? 'var(--flag)' : 'var(--ok)';
  const colorB = scoreDelta < 0 ? 'var(--ok)'   : 'var(--flag)';
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)', color: 'var(--text)' }}>
      ...
      <section style={{ padding: '32px 28px 16px', maxWidth: 1280, margin: '0 auto' }}>
        ...
        >
          {a.name.split(' ').slice(0, 1)} y {b.name.split(' ').slice(0, 1)}
        </h1>
        ...
        >
          ...
          <div style={{ marginTop: 18, textAlign: 'center' }}>
            ...
            <div
              ...
              style={{
                ...
                lineHeight: 1,
                fontWeight: 500,
                color: scoreDelta < 0 ? 'var(--flag)' : 'var(--ok)',
          ...
          >
            {a.dimensions.map((d, i) => {
              ...
              const shortLabel = (d.label ?? DIMENSION_LABELS[d.key]).split(' ')[0];
              return (
